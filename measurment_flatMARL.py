...
def parse_args():
    parser = argparse.ArgumentParser(description='Evaluate DongChen06 Flat MARL Baseline on SUMO')
...
<routes>
    ...
    <vTypeDistribution id="hdv_mixture">
        ...
        <vType id="hdv_aggressive" probability="0.4" carFollowModel="IDM" accel="3.5" decel="4.5" emergencyDecel="9.0" apparentDecel="4.5" tau="0.8" speedFactor="1.15" speedDev="0.1" lcCooperative="0.0" lcAssertive="5.0" color="0,0,255"/>
    </vTypeDistribution>
    ...
    def step(self, action_dict):
        ...
        for cav, action_idx in action_dict.items():
            try:
                ...
                traci.vehicle.setAcceleration(cav, accel, duration=1.0)
            except traci.exceptions.TraCIException:
                pass
...
def main():
    ...
    mappo = MAPPO(env=env_wrapper, state_dim=state_dim, action_dim=action_dim, num_cavs=NUM_CAVS,
    ...
    for lane_demand in lane_demands:
        for pr in prs:
            print(f"\n==================================================")
            print(f"EVALUATING DONGCHEN: Lane Demand {lane_demand} veh/h/lane | PR {pr*100:.0f}%")
            print(f"==================================================")
            
            generate_mixed_traffic_route(target_penetration_rate=pr, lane_demand_vph=lane_demand)
            
            # Master results store for aggregating across all seeds
            agg_results = {
            ...
            for seed_idx, current_seed in enumerate(test_seeds):
                ...
                traci_args = [current_binary, "-n", net_file, "-r", route_file, 
                              ...
                              # CRITICAL FIX: ENABLES REAL COLLISION PHYSICS IN SUMO
                ...
                if args.record_video:
                    ...
                    view_id = views[0] if views else "View #0"
                    frame_dir = f"temp_frames_DONGCHEN_ld{lane_demand}_p{int(pr*100)}_s{current_seed}"
                    if os.path.exists(frame_dir):
                        shutil.rmtree(frame_dir)
                    os.makedirs(frame_dir)
                
                # Per-seed metric tracking
                seed_metrics = {
                ...
                veh_track = {}
                completed_veh_track = {}  # Save vehicles that completed their route for comfort profiles
                active_cav_braking = {}  # Tracks continuous braking duration: {veh_id: step_count}
                ...
                while step < MAX_STEPS:
                    ...
                    # Catch collisions immediately after the step since they are removed
                    ...
                    for c_veh in collisions:
                        ...
                        # If a ramp vehicle crashes, record it as failed
                    ...
                    if candidates:
                        ...
                        for p, a in zip(worst_data['pos_history'], worst_data['accel_history']):
                            if 0 <= p <= 300: 
                                plot_pos.append(p)
                                plot_acc.append(a)
                        ax.plot(plot_pos, plot_acc, color='#ff7f0e', linewidth=1.5, label='Baseline (DongChen06)')
                ...
                accel_plot_filename = os.path.join(save_dir, f"comfort_profiles_DONGCHEN_ld{lane_demand}_p{int(pr*100)}_s{current_seed}.png")
                ...
                if args.record_video:
                    ...
                    if images:
                        ...
                        video_name = os.path.join(recordings_dir, f'DongChen_LaneD{lane_demand}_PR{int(pr*100)}_Seed{current_seed}.mp4')
                ...
                interventions_per_cav = seed_metrics['cav_interventions'] / max(1, num_cavs_in_seed)
                total_cav_active_steps = sum(seed_metrics['cav_active_steps'].values())
                duty_cycle = (seed_metrics['cav_interventions'] / max(1, total_cav_active_steps)) * 100.0
                
                agg_results['interventions_total'].append(seed_metrics['cav_interventions'])
                agg_results['interventions_per_cav'].append(interventions_per_cav)
                agg_results['intervention_duty_cycle'].append(duty_cycle)
            ...
            speed_array_filename = os.path.join(save_dir, f"speed_array_DONGCHEN_{lane_demand}vph_{int(pr*100)}PR.npy")
            decel_array_filename = os.path.join(save_dir, f"decel_array_DONGCHEN_{lane_demand}vph_{int(pr*100)}PR.npy")
            jerk_array_filename = os.path.join(save_dir, f"jerk_array_DONGCHEN_{lane_demand}vph_{int(pr*100)}PR.npy")
            ...
            plt.plot(range(MAX_STEPS), avg_speed_over_time, label='Baseline (DongChen06)', color='#ff7f0e') 
            ...
            plt.savefig(os.path.join(save_dir, f"speed_plot_DONGCHEN_{lane_demand}vph_{int(pr*100)}PR.png"), bbox_inches='tight', dpi=300)
            plt.close()
            
            # --- PLOT: MEAN DECELERATION ---
            plt.figure(figsize=(8, 4))
            plt.plot(range(MAX_STEPS), avg_decel_over_time, label='Baseline (DongChen06)', color='#d62728') 
            plt.xlabel('Simulation Step', fontsize=12)
            ...
            plt.tick_params(direction='in', which='both', right=True, top=True)
            plt.legend(frameon=False, loc='upper center', ncol=4, fontsize=11)
            plt.savefig(os.path.join(save_dir, f"decel_plot_DONGCHEN_{lane_demand}vph_{int(pr*100)}PR.png"), bbox_inches='tight', dpi=300)
            plt.close()
            
            # --- PLOT: MEAN ABSOLUTE JERK ---
            plt.figure(figsize=(8, 4))
            plt.plot(range(MAX_STEPS), avg_jerk_over_time, label='Baseline (DongChen06)', color='#2ca02c') 
            plt.xlabel('Simulation Step', fontsize=12)
            ...
            plt.tick_params(direction='in', which='both', right=True, top=True)
            plt.legend(frameon=False, loc='upper center', ncol=4, fontsize=11)
            plt.savefig(os.path.join(save_dir, f"jerk_plot_DONGCHEN_{lane_demand}vph_{int(pr*100)}PR.png"), bbox_inches='tight', dpi=300)
            plt.close()
            ...
            if all_raw_decels:
                ...
                cdf_filename = os.path.join(save_dir, f"cdf_decel_DONGCHEN_{lane_demand}vph_{int(pr*100)}PR.png")
            ...
            if all_raw_inference_times:
                ...
                inf_cdf_filename = os.path.join(save_dir, f"cdf_inference_DONGCHEN_{lane_demand}vph_{int(pr*100)}PR.png")
            ...
            report = f"""
==================================================
DONGCHEN06 BASELINE: {lane_demand} veh/h/lane | {pr*100:.0f}% PR (Over {num_seeds} Seeds)
...
"""
            ...
            output_filepath = os.path.join(save_dir, f"report_DONGCHEN_ld{lane_demand}vph_{int(pr*100)}PR.txt")
